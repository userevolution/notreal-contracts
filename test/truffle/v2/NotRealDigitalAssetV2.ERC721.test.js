...
contract('NotRealDigitalAssetV2 - ERC721Token', function (accounts) {
  ...
  const name = 'NotRealDigitalAsset';
  const symbol = 'NRDA';
    ...
    describe('mint', () => {
      ...
      describe('when successful', () => {
        ...
        it('emits a transfer event', async () => {
          ...
          logs[0].args.to.should.be.eq.BN(to);
          logs[0].args.tokenId.should.be.eq.BN(thirdTokenId);
        });
      });
    ...
    describe('burn', () => {
      ...
      describe('when successful', () => {
        ...
        it('emits a burn event', async () => {
          ...
          logs[1].args.to.should.be.equal(ZERO_ADDRESS);
          logs[1].args.tokenId.should.be.eq.BN(tokenId);
        });
      });
      ...
      describe('when there is a previous approval', () => {
        ...
        it('clears the approval', async () => {
          assertRevert(this.token.getApproved(tokenId),"ERC721: approved query for nonexistent token"); 
        });
      });
      ...
      const transferWasSuccessful = function ({owner, tokenId, approved}) {
        ...
        if (approved) {
          it('emit only a transfer event', async () => {
            ...
            logs[1].args.to.should.be.equal(this.to);
            logs[1].args.tokenId.should.be.eq.BN(tokenId);
          });
        } else {
      ...
      const shouldTransferTokensByUsers = function (transferFunction) {
        ...
        context('when sent to the owner', () => {
          ...
          it('emits only a transfer event', async () => {
            ...
            logs[1].event.should.be.equal('Transfer');
            logs[1].args.from.should.be.equal(owner);
            logs[1].args.to.should.be.equal(owner);
            logs[1].args.tokenId.should.be.eq.BN(tokenId);
          });
    ...
    describe('approve', () => {
      ...
      const itEmitsApprovalEvent = function (address) {
        it('emits an approval event', async () => {
          ...
          logs[0].args.tokenId.should.be.eq.BN(tokenId);
        });
      };
      ...
      context('when the operator willing to approve is not the owner', () => {
        ...
        context('when there is no operator approval set by the sender', () => {
          ...
          it('emits an approval event', async () => {
            ...
            logs[0].args.approved.should.be.true;
          });
        });
